...
const NewsList = () => {
    ...
    const fetchArticles = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:8000/api/articles/filter",
          {
            params: {
              page: currentPage,
              limit: itemsPerPage,
  ...
  const handleViewArticle = async (articleId) => {
    try {
      await axios.post(
        `http://localhost:8000/api/articles/view/${articleId}`,
        {},
          ...
          >
            ...
            {Object.keys(sortOptions).map((key, index) => (
              <option key={index} value={key}>
                {key}
              </option> // Keep value as key, not the query
            ))}
          </select>
        </div>
          ...
          {articles.map((article) => (
            <div
              ...
              className={`${
                viewMode === "list"
                  ? "flex space-x-4"
                  : "flex flex-col bg-white shadow-sm rounded-md overflow-hidden cursor-pointer"
              }`}
            >
              <div
                className={`${
                  viewMode === "list" ? "w-36 h-36" : "w-full h-48"
